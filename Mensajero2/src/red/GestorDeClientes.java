...
public class GestorDeClientes implements Runnable {
    ...
    public void run(){
        ...
        try{
            ...
            enviarMensajeAUsuario("Te damos la bienvenida al Servidor",Mensaje.MENSAJE_DEL_SERVIDOR);
            enviarMensajeAUsuario("Por favor ingresa un nombre de usuario:", Mensaje.MENSAJE_DEL_SERVIDOR);
                ...
                (mensaje = entradaDeMensajes.readLine()) != null
                    ...
                    if (servidor.nombreEnUso(mensaje)) {
                        enviarMensajeAUsuario(
                                "Ese nombre ya esta en uso. Prueba nuevamente",
                        ...
                        sinNombre = false;
                        enviarMensajeAUsuario("Hola, "+mensaje, Mensaje.MENSAJE_DEL_SERVIDOR);
                        enviarMensajeAUsuario(
                                "En este momento hay " 
            ...
            while ( 
                (mensaje = entradaDeMensajes.readLine()) != null // Chequeamos que no haya mensajes del usuario
                  ) 
            {
    ...
    /**
     * Permite enviarle un mensaje al usuario manejado por el gestor
     * @param msj 
     * @param tipo
     */
    public void enviarMensajeAUsuario(String msj, int tipo){
        
        Mensaje mensaje = new Mensaje(cliente.getUsuario(), msj,tipo);
